...
def apply_filter_from_config(img_rgb, config_path):
    ...
    if config.get("is_sobel", False):
        raise ValueError("Use um filtro de pré-processamento comum aqui, não Sobel.")
...
def compute_sobel_magnitude(img_rgb):
    gh = atrous_correlation_rgb(img_rgb, kernel=SOBEL_H, r=1, stride=1, activation="identity")
    gv = atrous_correlation_rgb(img_rgb, kernel=SOBEL_V, r=1, stride=1, activation="identity")
    mag = np.sqrt(gh.astype(np.float32) ** 2 + gv.astype(np.float32) ** 2)
    mag = np.mean(mag, axis=2)
    return histogram_stretch(mag)
...
def run_tree_count(input_path, output_dir, config_path):
    ...
    if cfg.get("use_sobel_validation", True):
        sobel_mag = compute_sobel_magnitude(img_smooth)
    else:
        sobel_mag = np.zeros((img_smooth.shape[0] - 2, img_smooth.shape[1] - 2), dtype=np.uint8)
    ...
    save_gray(os.path.join(output_dir, "03_sobel_magnitude.png"), sobel_mag)
